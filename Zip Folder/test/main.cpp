	...
	public :
		...
		void Dipost(){
			...
			cin>>d_amt;
				...
				cout<<"congrats Your Amount has Deposit Sucessfully."<<endl;
		...
		void Withdraw(){
			...
			if(Wdraw_Amount>Money){
				cout<<"Insufficent Amount";
				PlaySound(TEXT("insufficient _amount.wav"),NULL,SND_SYNC);
			}
			else{
				cout<<"Your Actual Amount Is : "<<Money<<endl;
		...
		void Wlc(){
			...
			PlaySound(TEXT("Bank.wav"),NULL,SND_SYNC);
			...
			if(c!='\0'){
				...
				switch(c){
					...
					case 'S' :
						...
						cout<<"\t\t\t\t  -------------------------------------------"<<endl;
						PlaySound(TEXT("Staff.wav"),NULL,SND_SYNC);
						...
						PlaySound(TEXT("call1.wav"),NULL,SND_SYNC);
						cin>>op;
						switch(op){
							case '1' :
								system("CLS");
